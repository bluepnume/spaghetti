...
module.exports = Class.extend({
	...
	handle: function(context, first) {
		...
		return q.try(function() {
			...
			// In this case all we can do is send the error on to the next middleware.
			if (frame.state === this.errorState)
				throw err;

			// Otherwise, let's log the error, and try entering the provided error state for the first time.
